...
> **2026-09-21 口径**：内容不含价格，价格在商品；「免费/付费 · 试看 · 下单方式」在首页装修的上架配置里决定。

1. 管理端 → **内容** → **线上课** → 编辑任意课程（看：基础信息 / **关联商品** / 状态与分享；内容侧已无价格字段）
2. 管理端 → **交易 · 商品列表** → **新建商品**（直接进「通过内容创建」；旁边另有「通过平台商品创建」）→ **弹窗多选「交付内容」**（可跨类型多选 = 组合商品）→ 给内容定价
3. 管理端 → **交易 · 店铺运营 · 首页装修** → 在「线上课」区块点 **配置** → 设 免费/付费 → 试看范围 → 下单方式（**跳转商品详情页** / **不跳转直接按商品金额下单**）→ 选默认成交商品 → 保存
   - **直播区块的配置弹窗不同**：售卖方式只读（选了成交商品=付费）、无试看、下单时机固定「进入直播间即付费」
4. 点 **预览 C 端首页** → 点进课程详情：`inline` 直接显示成交商品价并下单；`jump_goods` 点「去购买」进 **商品详情页** → 支付成功 → **我的 / 学习**
...
1. **SCRM 概览**：线索概览与简报  
2. **期次管理**：绑定视频号商品 + 承接助教（承接助教**按组织架构选择**，留资按商品分配）  
3. 侧栏按现网结构：全域获客 / 客户中心 / **用户运营** / 营销管理 / 内容中心 / 业务设置  
4. **直播促到** → **用户详情**打标 → **看板**

> 侧栏「用户运营」下另有 **用户事件营销（二期）**、**智能营销（二期）** 两个流程编排页（列表页 → 设计器，设计器为「节点面板 + LogicFlow 画布 + 属性面板」）。二期能力，仅作演示，不进 P0 验收。
