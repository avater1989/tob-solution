/* review-kit · annotate.js —— 可移植的页面评审标注层
 * 用法：Alt+E 进入/退出标注模式 → 点击元素 → 写意见 → 保存。
 * 每条标注记录三重锚点（页面路径 + CSS 选择器 + 元素文本/HTML 快照），
 * 供人或 AI 按锚点精确定位并修改代码。
 *
 * 通用性：不修改宿主页面 DOM，角标与高亮全部绘制在独立的 __ann_layer 图层；
 * 项目差异（内容根、外壳脚本、页面 key、接口前缀）全部通过 window.REVIEW_KIT 配置覆盖。
 * 详见同目录 README.md 与 ANNOTATION-SCHEMA.md。
 */
(function () {
  ...
  /* ---------------- 宿主配置（可选，全部有默认值） ----------------
   * 在加载本文件【之前】定义 window.REVIEW_KIT：
   * {
   *   serverBase: "/__review",        // 服务端接口前缀
   *   storeKey:   "proto:review:v1",  // localStorage 键
   *   pageRoot:   "/prototype/",      // 从路径中截取，作为 page 标识（如 admin/lives.html）
   *   pageStrip:  /^\/site\//,        // 或：直接去掉某前缀
   *   pageKey:    function(location){}, // 或：完全自定义
   *   roots:        [{sel, region}],           // 内容根：决定 region 与选择器起算点
   *   cssStopIds:   ["page-content"],          // 不作为 id 锚点的 id（由 root 指代）
   *   shells:       [{name, hints, container}],// 外壳脚本区域：region=shell，并给出要改的脚本名
   *   defaultType:     "copy",        // 新建标注默认类型
   *   defaultPriority: "P1",          // 新建标注默认优先级
   *   onToast:   function(msg){}      // 宿主自己的 toast；不填则尝试 window.Proto.toast
   * }
   */
  var CFG = window.REVIEW_KIT || {};
  var STORE_KEY = CFG.storeKey || "proto:review:v1";
  var SERVER = CFG.serverBase || "/__review";
  var CSS_STOP_IDS = CFG.cssStopIds || ["page-content", "header-actions"];
  var TYPES = [
    { id: "copy", label: "文案" },
    { id: "layout", label: "布局" },
  ...
  /* 宿主 toast：优先用配置的 onToast，其次 window.Proto.toast，都没有就静默 */
  function toast(msg) {
    if (typeof CFG.onToast === "function") {
      try { CFG.onToast(msg); } catch (e) {}
      return;
    }
    if (window.Proto && window.Proto.toast) window.Proto.toast(msg);
  }
  ...
  function pageKey() {
    if (typeof CFG.pageKey === "function") return CFG.pageKey(location);
    var p = decodeURIComponent(location.pathname);
    if (CFG.pageRoot) {
      var i = p.indexOf(CFG.pageRoot);
      if (i >= 0) return p.slice(i + CFG.pageRoot.length);
    }
    if (CFG.pageStrip) p = p.replace(CFG.pageStrip, "");
  ...
  function cssPath(el, stop) {
    ...
    while (node && node.nodeType === 1 && node !== stop && node !== document.documentElement) {
      if (node.id && node.id.indexOf("__ann") !== 0 && CSS_STOP_IDS.indexOf(node.id) === -1) {
  ...
  /* 由外壳脚本生成的区域（顶栏 / 侧栏 / 底部 tabbar）：
     记为 region=shell，并指出「要改哪个脚本」，避免让人去改页面 HTML。 */
  var SHELLS = CFG.shells || [
  ...
  var SHELL_SELECTOR = SHELLS.filter(function (s) { return s.container; })
    .map(function (s) { return "." + s.container; }).join(",");
  ...
  function anchorRoot(el) {
    ...
    if (SHELL_SELECTOR && el.closest && el.closest(SHELL_SELECTOR)) return { root: null, region: "shell", sel: "" };
    return { root: null, region: "page", sel: "" };
  }

  function hitAny(cls, list) {
    for (var i = 0; i < list.length; i++) {
      if (new RegExp("\\b" + list[i] + "\\b").test(cls)) return true;
    }
    return false;
  }
  ...
  function shellOwner(el) {
    ...
    for (var i = 0; i < SHELLS.length; i++) {
      if (SHELLS[i].hints && hitAny(cls, SHELLS[i].hints)) return SHELLS[i].name;
    }
    var r = SHELL_SELECTOR && el.closest ? el.closest(SHELL_SELECTOR) : null;
    if (r) {
      var c = r.getAttribute("class") || "";
      for (var j = 0; j < SHELLS.length; j++) {
        if (SHELLS[j].container && new RegExp("\\b" + SHELLS[j].container + "\\b").test(c)) return SHELLS[j].name;
      }
    }
    return "";
  }
  ...
  function openComposer(item, el) {
    ...
    if (editing) {
      ...
      state.editId = null;
      state.draft = {
        type: CFG.defaultType || "copy",
        priority: CFG.defaultPriority || "P1",
        comment: ""
      };
    }
