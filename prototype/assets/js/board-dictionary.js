/**
 * 数据看板指标字典（P2）— 集中维护，供口径说明与指标 tip 使用
 */
(function (global) {
  var METRICS = [
    {
    ...
    },
    {
      ...
      name: "期次入池人数",
      category: "期次经营",
      short: "归属当前招生期次的首次入池去重用户数",
      formula: "COUNT(DISTINCT 期次内入池用户)",
      numerator: "期次入池用户",
      denominator: "—",
      scope: "受日期 / 渠道 / 当前期次",
      dedupe: "按用户去重",
      ...
      caliber: "期次主口径",
      update: "随入池事件",
      note: "期次看板默认锁定单个期次，不把「全部期次」当作复盘对象。"
    },
    {
      id: "term_pay_users",
      name: "期次支付人数",
      category: "期次经营",
      short: "可归因到当前期次线索链路的支付去重人数",
      formula: "COUNT(DISTINCT 可归因支付用户)",
      numerator: "可归因支付用户",
      denominator: "—",
      scope: "受日期 / 渠道 / 当前期次",
      dedupe: "按用户去重",
      refund: "不含退款回冲支付人数（原型）",
      caliber: "可归因",
      update: "随支付事件",
      note: "与全量成交分区固定；筛选不改变口径定义。"
    },
    {
      id: "term_gmv",
      name: "期次成交",
      category: "期次经营",
      short: "期次关联订单的成交结果（全量或可归因分区分别展示）",
      formula: "SUM(订单实付)",
      numerator: "订单实付",
      denominator: "—",
      scope: "可归因受期次/渠道；全量以日期与成交类型为主",
      dedupe: "按订单",
      refund: "退款单独展示",
      caliber: "分区固定",
      update: "随订单",
      note: "映射覆盖不全时显示「—」，不用 0 冒充。"
    },
    {
      id: "term_goal_rate",
      name: "期次目标完成率",
      category: "期次经营",
      short: "当前值相对期次目标的完成比例",
      formula: "当前值 ÷ 目标值",
      numerator: "当前值",
      denominator: "期次目标",
      scope: "当前期次目标集中维护",
    ...
    },
    {
      ...
      category: "期次经营",
      short: "与配置的可比期次同期比较",
      formula: "数量用百分比差；比例用百分点差",
      numerator: "—",
      denominator: "—",
      scope: "当前期次 vs compareTermId",
      dedupe: "—",
      refund: "—",
      caliber: "环比说明",
      update: "按复盘时点",
      note: "无可比期次时显示「暂无可比期次」。"
    },
    {
      id: "term_related_lives",
      name: "关联直播场次",
      category: "期次经营",
      short: "期次主数据配置的关联直播场次集合",
      formula: "relatedLiveIds",
      numerator: "—",
      denominator: "—",
      scope: "当前期次",
      dedupe: "按场次 ID",
    ...
    },
    {
      ...
      name: "期次归因覆盖范围",
      category: "期次经营",
      short: "期次订单/支付可映射到线索归因的覆盖说明",
      formula: "可映射订单 ÷ 期次相关订单（演示）",
      numerator: "可映射",
      denominator: "期次相关订单",
      scope: "当前期次",
      dedupe: "按订单",
  ...
  var PAGE_METRIC_IDS = {
    ...
    term_review: ["term_pool", "term_pay_users", "term_gmv", "term_goal_rate", "term_vs_prior", "term_related_lives", "term_attr_cover", "wecom_rate", "attend_rate", "cover_rate"]
  };
