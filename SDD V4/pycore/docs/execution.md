...
print(result.data)       # "Saved: HELLO WORLD"
...
class PipelineFlow(BaseFlow):
    ...
    async def execute(self, input_data) -> FlowResult:
        ...
        # 阶段2
        result = await self.process_flow.run(result.data)
        if not result:
            return result
...
async def main():
    ...
    async with ExecutionContext.scope(request_id="req_123"):
        ...
        if result.success:
            logger.info(f"Data flow completed: total={result.data['total']}")

            # 运行并行流程
            notify_result = await notify_flow.run(result.data)
