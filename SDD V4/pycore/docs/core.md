...
```dotenv
DEBUG=true
SECRET_KEY=replace-in-local-config
ALLOWED_HOSTS=["localhost","example.com"]
```

键名会转换为小写并交给 Pydantic 校验。数组和对象使用 JSON 语法。加载器读取文件但不会把值写入进程环境。
...
默认不会读取系统环境变量。只有调用方明确传入 `use_env=True` 时，`PYCORE_` 前缀变量才会覆盖文件配置：

```python
config.load(AppSettings, ".env", use_env=True)
```
...
try:
    result = await registry.execute("unknown_plugin", data=data)
