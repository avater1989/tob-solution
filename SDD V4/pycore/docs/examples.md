    ...
    async def step(self) -> str:
        ...
        if response.has_tool_calls:
            ...
            for tool_call in response.tool_calls:
                ...
                result = await self.plugins.execute(tool_call.name, **args)
                rendered = result.data if result else result.error
                results.append(f"{tool_call.name}: {rendered}")
...
class ValidatorPlugin(BasePlugin):
    ...
    async def execute(self, item: ItemCreate, **kwargs) -> PluginResult:
        if not item.name or len(item.name) < 2:
            return self.fail("Name too short")
        if item.price < 0:
            return self.fail("Price must be positive")
...
async def create_item(data: ItemCreate):
    ...
    # 验证
    result = await plugins.execute("validator", item=data)
    if not result.success:
        response, _ = error_response(result.error or "Validation failed", "VALIDATION_ERROR", 400)
        return response
