...
class DatabasePlugin(BasePlugin):
    ...
    async def execute(self, query: str, **kwargs) -> PluginResult:
        ...
        if not self._connection:
            return self.fail("Database not connected")
        ...
        try:
            ...
            return self.fail(f"Query failed: {e}")
```
...
print(result.error)   # "Plugin 'unknown' not found"
```
...
class MyPlugin(BasePlugin):
    async def execute(self, **kwargs) -> PluginResult:
        ...
        # 失败结果
        return self.fail("Something went wrong")
...
class SafePlugin(BasePlugin):
    ...
    async def execute(self, **kwargs) -> PluginResult:
        try:
            ...
            return self.fail(f"Invalid value: {e}")

        except ConnectionError as e:
            return self.fail(f"Connection failed: {e}")

        except Exception as e:
            # 记录意外错误
            self._logger.exception("Unexpected error")
            return self.fail(f"Unexpected error: {e}")
```
