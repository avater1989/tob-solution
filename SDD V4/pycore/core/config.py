...
class DotEnvConfigLoader(ConfigLoader):
    """Deterministic .env file loader that does not mutate process state."""

    def supports(self, path: Path) -> bool:
        return path.suffix.lower() in (".env",)
    ...
    def load(self, path: Path) -> dict[str, Any]:
        ...
        for line_number, raw_line in enumerate(lines, start=1):
            ...
            if "=" not in line:
                raise ConfigurationError(
                    f"Invalid .env entry at line {line_number}",
                    config_path=str(path),
                )
            ...
            if not key or not key.replace("_", "").isalnum():
                raise ConfigurationError(
                    f"Invalid .env key at line {line_number}",
                    config_path=str(path),
                )
            ...
            if len(value) >= 2 and value[0] == value[-1] == '"':
                try:
                    ...
                    raise ConfigurationError(
                        f"Invalid quoted .env value at line {line_number}: {e}",
                        config_path=str(path),
                    ) from e
            elif len(value) >= 2 and value[0] == value[-1] == "'":
                parsed = value[1:-1]
            elif value.startswith(("[", "{")) or value in ("true", "false", "null"):
                try:
    ...
    def load(
        ...
        *,
        ...
        参数：
            ...
            use_env: 是否显式允许 PYCORE_ 环境变量覆盖，默认关闭
        """
